...
class DiffusionDet(nn.Module):
    ...
    def ddim_sample(self, batched_inputs, backbone_feats, images_whwh, images, clip_denoised=True, do_postprocess=True):
        ...
        diff_boxes_yolo_batch = []
        for batch_ins in batched_inputs:
            image_id = batch_ins["image_id"]
            yolo_pred_path = glob(f"/output/yolov6_train_diffusion_output/**/{image_id}.txt", recursive=True)
            if image_id is not None and len(yolo_pred_path) > 0:
                with open(yolo_pred_path[0], "r") as f:
                    lines = f.readlines()
                    f.close()
                diff_boxes_yolo = []
                for line in lines:
                    line = line.split()
                    diff_boxes_yolo.append([float(line[1]), float(line[2]), float(line[3]), float(line[4])])
                diff_boxes_yolo = torch.tensor(diff_boxes_yolo, device=self.device)
                diff_boxes_yolo = diff_boxes_yolo.repeat((self.num_proposals // diff_boxes_yolo.shape[0]) + 1, 1, 1).view(-1, 4)[:self.num_proposals]
                diff_boxes_yolo_batch.append(diff_boxes_yolo)
        
        if len(diff_boxes_yolo_batch) > 0:
            img = torch.stack(diff_boxes_yolo_batch, dim=0)
        else:
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        if self.training:
            ...
            targets, x_boxes, noises, t = self.prepare_targets(gt_instances, image_ids=image_ids)
            t = t.squeeze(-1)
            x_boxes = x_boxes * images_whwh[:, None, :]
    ...
    def prepare_diffusion_concat(self, gt_boxes, image_id=None):
        """
        :param gt_boxes: (cx, cy, w, h), normalized
        :param num_proposals:
        """
        ...
        if image_id is not None and len(yolo_pred_path) > 0:
            ...
            for line in lines:
                line = line.split()
                diff_boxes.append([float(line[1]), float(line[2]), float(line[3]), float(line[4])])
            diff_boxes = torch.tensor(diff_boxes, device=self.device)
            diff_boxes = box_cxcywh_to_xyxy(diff_boxes)
            diff_boxes = diff_boxes.repeat((self.num_proposals // diff_boxes.shape[0]) + 1, 1, 1).view(-1, 4)[:self.num_proposals]
    ...
    def prepare_targets(self, targets, image_ids=None):
        new_targets = []
        diffused_boxes = []
        noises = []
        ts = []
        for idx, targets_per_image in enumerate(targets):
            ...
            d_boxes, d_noise, d_t = self.prepare_diffusion_concat(gt_boxes, image_id=image_ids[idx])
